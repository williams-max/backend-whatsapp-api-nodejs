...
class WhatsAppInstance {
    ...
    async init() {
        ...
        //   console.log("mongo ",mongoClient.db('whatsapp-api').collection(this.key))

        const { state, saveCreds } = await useMongoDBAuthState(this.collection)

        //console.log("informartion get mongo state",state)

        console.log("informartion get mongo saveCred",saveCreds)
    ...
    async sendContactMessage(number,message ) {
        console.log('paso 2',number, message)
    ...
    async createNewGroup(name, users) {
        ...
        try {
            const group = await this.instance.sock?.
            groupCreate("My Fab Group", [
